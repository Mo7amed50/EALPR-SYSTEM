...
@app.route('/api/analytics/visitor-stats')
@login_required
def get_visitor_stats():
    try:
        ...
        # Calculate total visitors and status counts
        ...
        # Get visitor traffic for the last 7 days with status breakdown
        daily_traffic = []
        daily_status = []
        for i in range(7):
            ...
            # Total count
            count = Visitor.objects(
                entry_datetime_utc__gte=day_start,
                entry_datetime_utc__lt=day_end
            ).count()
            
            # Status breakdown
            auth_count = Visitor.objects(entry_datetime_utc__gte=day_start, 
                                       entry_datetime_utc__lt=day_end, 
                                       status='authorized').count()
            unauth_count = Visitor.objects(entry_datetime_utc__gte=day_start, 
                                         entry_datetime_utc__lt=day_end, 
                                         status='unauthorized').count()
            pending_count = Visitor.objects(entry_datetime_utc__gte=day_start, 
                                          entry_datetime_utc__lt=day_end, 
        ...
        # Get peak hours with status breakdown
        hour_counts = {}
        for hour in range(24):
            hour_stats = {
                'total': Visitor.objects(
                    entry_datetime_utc__gte=month_start,
                    entry_datetime_utc__lt=end_date,
                    entry_time__startswith=f"{hour:02d}:"
                ).count(),
                'authorized': Visitor.objects(
                    entry_datetime_utc__gte=month_start,
                    entry_datetime_utc__lt=end_date,
                    entry_time__startswith=f"{hour:02d}:",
                    status='authorized'
                ).count(),
                'unauthorized': Visitor.objects(
                    entry_datetime_utc__gte=month_start,
                    entry_datetime_utc__lt=end_date,
                    entry_time__startswith=f"{hour:02d}:",
                    status='unauthorized'
                ).count(),
                'pending': Visitor.objects(
                    entry_datetime_utc__gte=month_start,
                    entry_datetime_utc__lt=end_date,
                    entry_time__startswith=f"{hour:02d}:",
                    status='pending'
            ...
            hour_counts[f"{hour:02d}:00"] = hour_stats
        ...
        app.logger.error(f"Error getting visitor stats: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'message': str(e)
        }), 500
...
def export_analytics_excel():
    try:
        ...
        for column in ['A', 'B']:
            ws_summary.column_dimensions[column].width = 20

        # Daily Traffic Sheet
        ws_daily = wb.create_sheet("Daily Traffic")
        daily_headers = ['Date', 'Day', 'Total', 'Authorized', 'Unauthorized', 'Pending']
        ...
        for i in range(30):  # Last 30 days
            ...
            total = Visitor.objects(entry_datetime_utc__gte=day_start, entry_datetime_utc__lt=day_end).count()
        ...
        # Style the daily traffic sheet
        for cell in ws_daily[1]:
            cell.font = Font(bold=True)
        for column in ['A', 'B', 'C', 'D', 'E', 'F']:
            ws_daily.column_dimensions[column].width = 15

        # Hourly Traffic Sheet
        ws_hourly = wb.create_sheet("Hourly Traffic")
        hourly_headers = ['Hour', 'Total', 'Authorized', 'Unauthorized', 'Pending']
        ...
        for hour in range(24):
            ...
            auth = Visitor.objects(entry_datetime_utc__gte=month_start, entry_time__startswith=f"{hour:02d}:", status='authorized').count()
            unauth = Visitor.objects(entry_datetime_utc__gte=month_start, entry_time__startswith=f"{hour:02d}:", status='unauthorized').count()
            pending = Visitor.objects(entry_datetime_utc__gte=month_start, entry_time__startswith=f"{hour:02d}:", status='pending').count()
            
            ws_hourly.append([hour_str, total, auth, unauth, pending])

        # Style the hourly traffic sheet
        for cell in ws_hourly[1]:
            cell.font = Font(bold=True)
        for column in ['A', 'B', 'C', 'D', 'E']:
            ws_hourly.column_dimensions[column].width = 15

        # Department Statistics Sheet
        ws_dept = wb.create_sheet("Department Statistics")
        dept_headers = ['Department', 'Total Visitors', 'Authorized', 'Unauthorized', 'Pending', 'Authorization Rate']
        ...
        for dept in departments:
            if dept:  # Skip empty department values
                ...
                pending = Visitor.objects(responsible_department=dept, status='pending').count()
                auth_rate = f"{(auth / total * 100) if total > 0 else 0:.1f}%"
                
                ws_dept.append([dept, total, auth, unauth, pending, auth_rate])

        # Style the department statistics sheet
        for cell in ws_dept[1]:
            cell.font = Font(bold=True)
        for column in ['A', 'B', 'C', 'D', 'E', 'F']:
            ws_dept.column_dimensions[column].width = 20
        ...
        return Response(
            ...
            mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            headers={
                'Content-Disposition': f'attachment; filename=visitor_analytics_{end_date.strftime("%Y%m%d")}.xlsx'
            }
        )

    except Exception as e:
        app.logger.error(f"Error exporting analytics to Excel: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'message': str(e)
        }), 500

@app.route('/api/reports/<type>')
@login_required
def export_report(type):
    ...
    try:
        ...
        app.logger.error(f"Error generating {type} report: {str(e)}", exc_info=True)
        if excel_file:
            try:
                excel_file.close()
            except:
